...
Pytest API 和内建 fixtures
================================================


本页面的大部分信息已移至 :ref:`api-reference`。

有关插件钩子和其他对象的信息，请参阅 :ref:`plugins`。

有关 ``pytest.mark`` 机制的信息，请参阅 :ref:`mark`。

有关 fixtures 的信息，请参阅 :ref:`fixtures`。要查看可用 fixtures 的完整列表（添加 :option:`-v` 选项以同时查看带有前导 ``_`` 的 fixtures），请键入：
